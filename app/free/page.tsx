    ...
    <div className="min-h-screen bg-[#FBF7F2] overflow-x-hidden">
      ...
      <main className="pt-24 pb-16">
        <div className="mx-auto max-w-6xl px-4 md:px-8">
          <div className="text-xs uppercase tracking-widest text-neutral-500">Free trial</div>
          <h1 className="mt-3 text-2xl sm:text-3xl md:text-4xl font-bold text-neutral-900">Book your free 10-minute session.</h1>
          <p className="mt-4 text-neutral-600 max-w-2xl leading-relaxed text-sm sm:text-base">
            A complimentary 10-minute 1:1 diagnostic. Pick a coach and choose an available slot. No obligation, no group class.
          </p>

          {loading ? (
            <div className="mt-10 text-neutral-500">Loading…</div>
          ) : coaches.length === 0 ? (
              ...
              <div>
                ...
                {coach && (
                  <div className="mt-4">
                    ...
                    {selectedSlot && coachId && date && <BookingForm coachId={coachId} coachEmail={coach?.email} coachName={coach?.name} date={date} slot={selectedSlot} sessionType="free" onDone={refresh} />}
                  </div>
                )}
              </div>
            </div>
